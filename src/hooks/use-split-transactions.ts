...
export const useSplitTransactions = () => {
  ...
  const asyncSetSplitTransactions = async (transactionId: number, splitTransactions: SplitTransaction[]) => {
    try {
      await setSplitTransaction(auth.accessToken, transactionId, splitTransactions)
    } catch (error) {
