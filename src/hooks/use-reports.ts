...
export const useReports = () => {
  const auth = useAuth()
  const [expenseTagReport, setExpenseTagReport] = useState(
    [] as ExpenseTagReport[]
  )
  const [expenseTrendReport, setExpenseTrendReport] = useState(
    [] as ExpenseTrendReport[]
  )
  ...
  const asyncFetchReportTransactions = async (filters?: ReportFilterProps) => {
    ...
    await fetchReportTransactions(auth.accessToken, filters)
      .then(response => {
        setExpenseTagReport(
          response.items?.map((item: ExpenseTagReport) => {
            return {
              ...item,
              tag: item.tag ?? {
                id: 0,
  ...
  const asyncFetchReportTrends = async (filters?: ReportFilterProps) => {
    ...
    await fetchReportTrends(auth.accessToken, filters)
      .then(response => {
        setExpenseTrendReport(
          response.items?.map((item: ExpenseTrendReport) => {
            return {
              ...item,
